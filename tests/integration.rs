...
fn test_bm25_relevant_scores_higher() {
    ...
    assert!(score_relevant > score_irrelevant, "Relevant doc should score higher");
}
...
async fn test_sqlite_store_insert_and_search() {
    ...
    let chunks = vec![
        Chunk::new("c1".into(), "API keys never expire and remain valid".into(), "security.md".into(), 0),
        Chunk::new("c2".into(), "API key validity 12 months renewed".into(), "pricing.md".into(), 0),
        Chunk::new("c3".into(), "Connection timeout is 30 seconds".into(), "api-v2.md".into(), 0),
    ];
...
async fn test_engine_index_and_query() {
    ...
    assert!(!query_result.sources.is_empty(), "Should find relevant sources");
    eprintln!("Query returned {} sources", query_result.sources.len());
    for s in &query_result.sources {
        eprintln!("  [{:.2}] {} — {}...", s.score, s.document, &s.content[..s.content.len().min(60)]);
    }

    // At least one result should mention timeout
    let has_timeout = query_result.sources.iter().any(|s| s.content.to_lowercase().contains("timeout"));
...
async fn test_engine_conflicting_sources() {
    ...
    let result = engine.query("API keys expire validity months", 10).await.unwrap();
    eprintln!("Conflict query returned {} sources", result.sources.len());
    for s in &result.sources {
        eprintln!("  [{:.2}] {} — {}...", s.score, s.document, &s.content[..s.content.len().min(80)]);
    }
...
fn test_query_no_results_for_unrelated() {
    let chunks = vec![
        Chunk::new("c1".into(), "API documentation for cloud services".into(), "doc.md".into(), 0),
        Chunk::new("c2".into(), "Rate limiting and authentication".into(), "doc.md".into(), 1),
    ];
    let results = retrieve(&chunks, "quantum physics entanglement", 5);

    // Unrelated query should produce low scores
    if !results.is_empty() {
        assert!(results[0].score < 0.5, "Unrelated query should have low scores");
    }
}
